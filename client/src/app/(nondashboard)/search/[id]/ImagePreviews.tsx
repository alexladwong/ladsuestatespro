...
const ImagePreviews = ({ images }: ImagePreviewsProps) => {
  const [curLADSUImageIndex, setCurLADSUImageIndex] = useState(0);

  const handlePrev = () => {
    setCurLADSUImageIndex((prev) => (prev === 0 ? images.length - 1 : prev - 1));
  };

  const handleNext = () => {
    setCurLADSUImageIndex((prev) => (prev === images.length - 1 ? 0 : prev + 1));
  };
      ...
      {images.map((image, index) => (
        <div
          ...
          className={`absolute inset-0 transition-opacity duration-500 ease-in-out ${
            index === curLADSUImageIndex ? "opacity-100" : "opacity-0"
          }`}
        >
          <Image
            src={image}
