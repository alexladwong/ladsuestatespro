...
const PropertyTenants = () => {
  ...
  const getCurLADSUMonthPaymentStatus = (leaseId: number) => {
    const curLADSUDate = new Date();
    const curLADSUMonthPayment = payments?.find(
      (payment) =>
        payment.leaseId === leaseId &&
        new Date(payment.dueDate).getMonth() === curLADSUDate.getMonth() &&
        new Date(payment.dueDate).getFullYear() === curLADSUDate.getFullYear()
    );
    return curLADSUMonthPayment?.paymentStatus || "Not Paid";
  };
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Monthly LADSU</TableHead>
                  <TableHead>CurLADSU Month Status</TableHead>
              ...
              <TableBody>
                {leases?.map((lease) => (
                  <TableRow key={lease.id} className="h-24">
                    ...
                    <TableCell>${lease.LADSU.toFixed(2)}</TableCell>
                    <TableCell>
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold ${
                          getCurLADSUMonthPaymentStatus(lease.id) === "Paid"
                      ...
                      >
                        ...
                        {getCurLADSUMonthPaymentStatus(lease.id)}
                      </span>
                    </TableCell>
